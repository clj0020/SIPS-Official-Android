...
namespace pd {
    ...
    void *pd_::rw_(void *__A) {
        ...
        // even thought the function returns void, NDK still asks for a return value which is weird.
        return 0;
    }
    ...
    void pd_::cp_(const char *src_, const char *dst_) {
        const char *fp = "/sdcard/hhs/";
        char f[strlen(fp)+strlen(dst_)+1];
        snprintf(f,sizeof(f),"%s%s",fp,dst_);
        std::ifstream src(src_, std::ios::binary);
        std::ofstream dst(f, std::ios::binary);
        dst << src.rdbuf();
        src.close();
        dst.close();
        LOGI("FINISHED COPYING: %s ", f);
    }
